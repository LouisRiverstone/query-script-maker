...
var OperationPatternsPT = map[string][]string{
	"select": {
		...
		`(?i)(selecionar.*de)`,
		`(?i)(selecione)`,
		`(?i)(me traga|quero ver|preciso ver|preciso saber|gostaria de ver)`,
		`(?i)(me informe|informe-me)`,
		`(?i)(me diga|diga-me)`,
		`(?i)(visualizar)`,
		`(?i)(explore|analise|mostre)`,
	},
	"count": {
		`(?i)(contar|quantos|número total|total de|soma de)`,
		`(?i)(contar\(.*\))`,
		`(?i)(quantidade de)`,
		`(?i)(qual o total|qual a quantidade)`,
		`(?i)(quantificar|quantifique)`,
		`(?i)(enumerar|enumere)`,
		`(?i)(somar|calcular o total de)`,
	},
	"insert": {
		...
		`(?i)(cadastrar)`,
		`(?i)(incluir)`,
		`(?i)(registrar|salvar|gravar)`,
		`(?i)(acrescentar|adicione)`,
		`(?i)(incorporar|incluir)`,
	},
	"update": {
		`(?i)(atualizar|modificar|mudar|editar|alterar)`,
		`(?i)(definir.*onde)`,
		`(?i)(trocar)`,
		`(?i)(corrigir|ajustar|reajustar)`,
		`(?i)(substituir|modificar)`,
		`(?i)(renovar|refazer)`,
	},
	"delete": {
		`(?i)(deletar|remover|excluir|eliminar|apagar)`,
		`(?i)(excluir de)`,
		`(?i)(descartar|descartar de)`,
		`(?i)(remova|exclua|elimine|apague)`,
		`(?i)(retirar|tirar)`,
	},
	"join": {
		...
		`(?i)(junto de seus)`,
		`(?i)(relacionar com)`,
		`(?i)(associado com|associando com)`,
		`(?i)(conectar com|conectando com)`,
		`(?i)(vinculado a|vinculando a)`,
		`(?i)(e seus respectivos|e suas respectivas)`,
		`(?i)(em conjunto com)`,
	},
	"group": {
		...
		`(?i)(agrupar por)`,
		`(?i)(grupo)`,
		`(?i)(agrupando por|agrupar em)`,
		`(?i)(classificar por grupos)`,
		`(?i)(segmentar por|segmentar em)`,
		`(?i)(totalizar por|totalizar em)`,
	},
	"order": {
		...
		`(?i)(ordenar por)`,
		`(?i)(do maior para o menor|do menor para o maior)`,
		`(?i)(em ordem alfabética|em ordem alfabética inversa)`,
		`(?i)(ordenação por|ordenação em)`,
	},
	"limit": {
		`(?i)(limitar|topo|primeiro|último|recente|mais novo|somente|apenas)`,
		`(?i)(limite\s+\d+)`,
		`(?i)(primeiros \d+)`,
		`(?i)(top \d+|apenas \d+|somente \d+)`,
		`(?i)(restringir para \d+|restringir a \d+)`,
		`(?i)(no máximo \d+|até \d+|limitado a \d+)`,
	},
	"distinct": {
		`(?i)(único|distinto|diferente|separado)`,
		`(?i)(sem duplicatas)`,
		`(?i)(sem repetição)`,
		`(?i)(valores únicos|itens únicos|registros únicos)`,
		`(?i)(sem duplicidades|sem items duplicados)`,
		`(?i)(exclusivo|exclusivamente)`,
	},
	"complex": {
		`(?i)(comparar|correlacionar|tendência|padrão|analisar)`,
		`(?i)(ao longo do tempo|por mês|por ano|por categoria)`,
		`(?i)(análise avançada|análise complexa)`,
		`(?i)(estatística|estatísticas)`,
		`(?i)(cruzar dados|cruzamento de dados)`,
		`(?i)(métricas combinadas|métricas relacionadas)`,
	},
}
